...
class ReportsApiTestCase(APITestCase):
    def setUp(self):
        ...
        self.build_booking = BookingFactory.create(user=self.admin, check_in=datetime.date(year=2024, month=4, day=7),
                                                   room=self.room1)
        self.build_booking = BookingFactory.create(user=self.admin, check_in=datetime.date(year=2024, month=4, day=7),
                                                   room=self.room2)

        self.hotel_rates = self.generate_reviews(self.hotel1, self.content_type_hotel, 5)
    ...
    def generate_reviews(object, content_type, n):
        ...
        for _ in range(n):
            ...
            abstract_review = AbstractReviewFactory.create(user=user, content_type=content_type,
                                                           object_id=object.pk)
    ...
    def test_hotel_report_generation(self):
        ...
        get_report = self.client.get(reverse('reports:hotels-hotel-reports', kwargs={'pk': self.hotel1.pk}),
                                     format='json')
        ...
        self.assertEqual(get_report.data['hotel_rating'], sum(self.hotel_rates) / len(self.hotel_rates))
        self.assertEqual(get_report.data['rooms_rating'], sum(self.room_rates) / len(self.room_rates))

    def test_booking_reports_generation(self):
        self.client.login(username=self.admin.username, password='lxm712HJKHK')

        get_report = self.client.get(reverse('reports:hotels-booking-reports', kwargs={'pk': self.hotel1.pk}),
                                     format='json')

        self.assertEqual(get_report.status_code, status.HTTP_201_CREATED)
        self.assertEqual(get_report.data['count_booking'],
                         Booking.objects.filter(room__hotel__name=self.hotel1.name).count())

    def test_rooms_report_generation(self):
        self.client.login(username=self.admin.username, password='lxm712HJKHK')

        get_report = self.client.get(reverse('reports:hotels-get-rooms', kwargs={'pk': self.hotel1.pk}),
                                     format='json')
    ...
    def test_get_page_room_reports(self):
        ...
        get_report = self.client.get(reverse('reports:rooms-page-reports'), format='json')

        self.assertEqual(get_report.status_code, status.HTTP_201_CREATED)
        self.assertEqual(get_report.data['room_amount'], RoomResultsSetPagination.page_size)

    def test_retrieve_room_report_generation(self):
        self.client.login(username=self.admin.username, password='lxm712HJKHK')

        get_report = self.client.get(reverse('reports:rooms-detail', kwargs={'pk': self.room1.pk}), format='json')
