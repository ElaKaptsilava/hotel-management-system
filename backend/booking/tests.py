...
from hotel_management.factories import RoomFactory, LocationFactory, HotelFactory, UserFactory
...
class BookingApiTestCase(APITestCase):
    def setUp(self):
        ...
        self.build_booking = BookingFactory.build(user=self.admin, check_in=datetime.date(year=2024, month=4, day=7),
                                                  room=self.room)

    def test_booking_status_is_active(self):
        self.client.login(username=self.admin.username, password='kjbafg873ghdsas9881')

        post_booking = self.client.post(reverse('booking-management:bookings-list'), model_to_dict(self.build_booking),
                                        format='json')
    ...
    def test_booking_is_not_available(self):
        self.client.login(username=self.admin.username, password='kjbafg873ghdsas9881')
        booking = BookingFactory.create(user=self.admin, check_in=datetime.date(year=2024, month=4, day=7),
                                        room=self.room)

        post_booking = self.client.post(reverse('booking-management:bookings-list'), model_to_dict(self.build_booking),
                                        format='json')

        expected_message = (f'This room is occupied from {self.build_booking.check_in} '
